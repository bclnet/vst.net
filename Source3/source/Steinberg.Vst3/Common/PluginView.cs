...
namespace Steinberg.Vst3.Common
{
    /// <summary>
    ...
    public class CPluginView : IPlugView
    {
        ...
        public virtual TResult IsPlatformTypeSupported(string type)
            => kNotImplemented;
        ...
        public virtual TResult OnWheel(float distance)
            => kResultFalse;

        public virtual TResult OnKeyDown(char key, short keyMsg, short modifiers)
            => kResultFalse;

        public virtual TResult OnKeyUp(char key, short keyMsg, short modifiers)
            => kResultFalse;

        public virtual TResult OnSize(ref ViewRect newSize)
        {
            rect = newSize;
            return kResultTrue;
        }

        public virtual TResult GetSize(out ViewRect size)
        {
            size = rect;
            return kResultTrue;
        }

        public virtual TResult OnFocus(bool state)
            => kResultFalse;

        public virtual TResult SetFrame(IPlugFrame frame)
        {
            plugFrame = frame;
            return kResultTrue;
        }

        public virtual TResult CanResize()
            => kResultFalse;

        public virtual TResult CheckSizeConstraint(ref ViewRect rect)
            => kResultFalse;
