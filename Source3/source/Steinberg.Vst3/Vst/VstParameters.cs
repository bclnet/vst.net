...
namespace Steinberg.Vst3
{
    ...
    public class Parameter : ObservableObject
    {
        ...
        protected Parameter() { }
        public Parameter(ref ParameterInfo info)
        {
            this.info = info;
        ...
        /// Gets the info.
        /// </summary>
        public virtual ParameterInfo Info
            => info;
    ...
    public class RangeParameter : Parameter
    {
        ...
        public RangeParameter(ref ParameterInfo paramInfo, ParamValue min, ParamValue max)
            : base(ref paramInfo)
        {
            minPlain = min;
            maxPlain = max;
        ...
        public override string ToString(ParamValue valueNormalized)
            => info.StepCount > 1
                ? ((long)ToPlain(valueNormalized)).ToString()
                : base.ToString(ToPlain(valueNormalized));
        ...
        public override ParamValue ToPlain(ParamValue valueNormalized)
            => info.StepCount > 1
                ? FUtils.FromNormalized<ParamValue>(valueNormalized, info.StepCount) + Min
                : valueNormalized * (Max - Min) + Min;
        ...
        public override ParamValue ToNormalized(ParamValue plainValue)
            => info.StepCount > 1
                ? FUtils.ToNormalized<ParamValue>(plainValue - Min, info.StepCount)
                : (plainValue - Min) / (Max - Min);
    }
