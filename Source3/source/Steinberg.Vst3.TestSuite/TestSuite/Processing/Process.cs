    ...
    public unsafe class ProcessTest : TestEnh
    {
        ...
        protected virtual bool PrepareProcessing()
        {
            ...
            if (audioEffect.SetupProcessing(processSetup) == kResultOk)
            {
                ...
                for (var dir = BusDirection.Input; dir <= BusDirection.Output; dir++)
                {
                    ...
                    var audioBuffers = dir == BusDirection.Input ? processData._.InputsX : processData._.OutputsX;
                    if (!SetupBuffers(numBusses, audioBuffers, dir)) return false;

                    if (dir == BusDirection.Input)
                    {
                        processData._.NumInputs = numBusses;
                        processData._.InputsX = audioBuffers;
                    }
                    else
                    {
                        processData._.NumOutputs = numBusses;
                        processData._.OutputsX = audioBuffers;
                    }
                }
                return true;
            }
        ...
        protected bool SetupBuffers(ref AudioBusBuffers audioBuffers)
        {
            ...
            for (var chIdx = 0; chIdx < audioBuffers.NumChannels; chIdx++)
                ...
                {
                    if (audioBuffers.ChannelBuffers32 != null)
                    {
                        var channelBuffers32 = audioBuffers.ChannelBuffers32;
                ...
                {
                    if (audioBuffers.ChannelBuffers64 != null)
                    {
                        var channelBuffers64 = audioBuffers.ChannelBuffers64;
        ...
        {
            var ret = true;
            ret &= FreeBuffers(processData._.NumInputs, processData._.InputsX);
            ret &= FreeBuffers(processData._.NumOutputs, processData._.OutputsX);
        ...
        protected bool FreeBuffers(int numBuses, IntPtr buses)
        {
            ...
            for (var busIndex = 0; busIndex < numBuses; busIndex++)
                ...
                {
                    if (processSetup.SymbolicSampleSize == SymbolicSampleSizes.Sample32) Marshal.FreeHGlobal((IntPtr)buses2[busIndex].ChannelBuffers32[chIdx]);
                    else if (processSetup.SymbolicSampleSize == SymbolicSampleSizes.Sample64) Marshal.FreeHGlobal((IntPtr)buses2[busIndex].ChannelBuffers64[chIdx]);
                    else return false;
                }
            return true;
        }
