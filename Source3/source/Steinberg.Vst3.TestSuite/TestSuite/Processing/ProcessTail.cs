        ...
        protected override bool PreProcess(ITestResult testResult)
        {
            if (!_inSilenceInput)
            {
                ...
                for (var i = 0; i < processData._.NumOutputs; ++i)
                    for (var c = 0; c < processData._.Outputs[i].NumChannels; ++c)
                        if (processSetup.SymbolicSampleSize == SymbolicSampleSizes.Sample32)
                            Platform.memset((IntPtr)processData._.Outputs[i].ChannelBuffers32[c], 0, processData._.NumSamples * sizeof(float));
                        else
                            Platform.memset((IntPtr)processData._.Outputs[i].ChannelBuffers64[c], 0, processData._.NumSamples * sizeof(double));
                for (var i = 0; i < processData._.NumInputs; ++i)
                    for (var c = 0; c < processData._.Inputs[i].NumChannels; ++c)
                        if (processSetup.SymbolicSampleSize == SymbolicSampleSizes.Sample32)
                            Platform.memcpy((IntPtr)processData._.Inputs[i].ChannelBuffers32[c], (IntPtr)dataPtrFloat, processData._.NumSamples * sizeof(float));
                        else
                            Platform.memcpy((IntPtr)processData._.Inputs[i].ChannelBuffers64[c], (IntPtr)dataPtrDouble, processData._.NumSamples * sizeof(double));
            }
            else
            {
                ...
                for (var i = 0; i < processData._.NumInputs; ++i)
                    for (var c = 0; c < processData._.Inputs[i].NumChannels; ++c)
                        if (processSetup.SymbolicSampleSize == SymbolicSampleSizes.Sample32)
                            Platform.memset((IntPtr)processData._.Inputs[i].ChannelBuffers32[c], 0, processData._.NumSamples * sizeof(float));
                        else
                            Platform.memset((IntPtr)processData._.Inputs[i].ChannelBuffers64[c], 0, processData._.NumSamples * sizeof(double));
            }
                        ...
                        for (var c = 0; c < processData._.Outputs[i].NumChannels; ++c)
                        {
                            if (processSetup.SymbolicSampleSize == SymbolicSampleSizes.Sample32)
                            {
                                for (var s = start; s < end; ++s)
                                    if (Math.Abs(processData._.Outputs[i].ChannelBuffers32[c][s]) >= 1e-7)
                                    {
                            ...
                            {
                                for (var s = start; s < end; ++s)
                                    if (Math.Abs(processData._.Outputs[i].ChannelBuffers64[c][s]) >= 1e-7)
                                    {
                                        testResult.AddErrorMessage($"IAudioProcessor::process (..) generates non silent output for silent input for tail above {_tailSamples} samples.");
                                        return false;
                                    }
